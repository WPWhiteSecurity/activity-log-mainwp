...
class RHtmlFormatter extends RFormatter {
	...
	public function startGroup( $prefix = '' ) {
		...
		$maxDepth = Event_Ref::config( 'maxDepth' );
		...
		$expLvl = Event_Ref::config( 'expLvl' );
	...
	public function endRoot() {
		...
		if ( ($timeout = Event_Ref::getTimeoutPoint()) > 0 ) {
	...
	public static function getAssets() {
		...
		if ( Event_Ref::config( 'stylePath' ) !== false ) {
			?>
			<style>
				<?php readfile( str_replace( '{:dir}', __DIR__, Event_Ref::config( 'stylePath' ) ) ); ?>
			</style>
			<?php
		}

		if ( Event_Ref::config( 'scriptPath' ) !== false ) {
			?>
			<script>
				<?php readfile( str_replace( '{:dir}', __DIR__, Event_Ref::config( 'scriptPath' ) ) ); ?>
			</script>
			<?php
		}
