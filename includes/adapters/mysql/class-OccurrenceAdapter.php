...
use \WSAL\MainWPExtension\Adapters\OccurrenceInterface as OccurrenceInterface;
 ...
 * @package mwp-al-ext
 */
class Occurrence extends ActiveRecord implements OccurrenceInterface {
	...
	protected function prepareOccurrenceQuery( $query ) {
		...
		if ( ! empty( $conditions ) ) {
			$tmp            = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $this->connection );
			$s_where_clause = '';
