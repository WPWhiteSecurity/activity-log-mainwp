...
use \WSAL\MainWPExtension\Connector\AbstractConnector as AbstractConnector;
use \WSAL\MainWPExtension\Connector\ConnectorInterface as ConnectorInterface;
use \WSAL\MainWPExtension\Connector\wpdbCustom as wpdbCustom;
 ...
 * @package mwp-al-ext
 */
class MySQLDB extends AbstractConnector implements ConnectorInterface {
	 ...
	 * @return \WSAL\MainWPExtension\Adapters\MySQL\{class_name}
	 */
	...
	public function installAll( $exclude_options = false ) {
		...
		foreach ( glob( $this->getAdaptersDirectory() . DIRECTORY_SEPARATOR . '*.php' ) as $file ) {
			...
			$class_name = $this->getAdapterClassName( str_replace( array( 'class-', 'Adapter.php' ), '', $file_name ) );
	...
	private function GetIncreaseOccurrence() {
		...
		$occurrence_new = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $_wpdb );
	...
	public function MigrateMeta( $index, $limit ) {
		...
		$meta = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $wpdb );
		if ( ! $meta->IsInstalled() ) {
			$result['empty'] = true;
			return $result;
		}
		...
		if ( ! empty( $metadata ) ) {
			$meta_new = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $_wpdb );
	...
	public function MigrateOccurrence( $index, $limit ) {
		...
		$occurrence = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $wpdb );
	...
	public function MigrateBackOccurrence( $index, $limit ) {
		...
		// Insert data to WP.
		if ( ! empty( $occurrences ) ) {
			$occurrence_wp = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $wpdb );
	...
	public function MigrateBackMeta( $index, $limit ) {
		...
		// Insert data to WP.
		if ( ! empty( $metadata ) ) {
			$meta_wp = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $wpdb );
	...
	public function MirroringAlertsToDB( $args ) {
		...
		if ( ! empty( $occurrences ) ) {
			$occurrence_new = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $mirroring_db );
		...
		if ( ! empty( $first_occurrence_id ) ) {
			...
			if ( ! empty( $metadata ) ) {
				$meta_new = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $mirroring_db );
	...
	public function ArchiveOccurrence( $args ) {
		...
		if ( ! empty( $occurrences ) ) {
			...
			$occurrence_new = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $archive_db );
	...
	public function ArchiveMeta( $args ) {
		...
		// Insert data to Archive DB.
		if ( ! empty( $metadata ) ) {
			$meta_new = new \WSAL\MainWPExtension\Adapters\MySQL\Meta( $archive_db );
	...
	public function purge_activity() {
		...
		$occ       = new \WSAL\MainWPExtension\Adapters\MySQL\Occurrence( $wpdb );
