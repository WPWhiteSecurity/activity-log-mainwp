 ...
 * @package mwp-al-ext
 *
 * @fs_premium_only /extensions/, /assets/js/dist/reports/, /assets/js/dist/search/, /assets/js/src/reports/, /assets/js/src/search/
 */
...
if ( function_exists( 'almainwp_fs' ) ) {
    ...
    class Activity_Log {
        ...
        public function init_hooks() {
            register_activation_hook( __FILE__, [ $this, 'install_extension' ] ); // Installation routine.
            add_action( 'init', [ $this, 'mwpal_init' ] ); // Start listening to events.
            add_action( 'init', [ $this, 'load_mwpal_text_domain' ] );
            add_action( 'mwp_events_cleanup', [ $this, 'events_cleanup' ] ); // Schedule hook for refreshing events.
            add_filter( 'mainwp_getextensions', [ &$this, 'get_this_extension' ] );
            add_action( 'admin_init', [ &$this, 'redirect_on_activate' ] );
            add_filter( 'plugin_action_links_' . MWPAL_BASE_NAME, [ $this, 'add_plugin_page_links' ], 20, 1 );
            add_action( 'plugins_loaded', [ $this, 'load_mwpal_extension' ] );
            add_action( 'admin_notices', [ &$this, 'mainwp_error_notice' ] );
            add_action( 'mainwp_delete_site', [ $this, 'remove_almwp_child_when_removed_from_mwp' ] );
            add_action( 'mainwp_header_left', [ $this, 'custom_page_title' ] );
             ...
             */
            if ( function_exists( 'almainwp_fs' ) ) {
                almainwp_fs()->add_action( 'after_account_connection', [ $this, 'account_connection_set' ] );
                almainwp_fs()->add_filter(
                    'plugin_icon',
        ...
        public function get_this_extension( $plugins ) {
            $plugins[] = [
                ...
                'mainwp'   => false,
                'callback' => [ &$this, 'display_extension' ],
